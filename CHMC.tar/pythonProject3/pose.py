...
import torch as t
import theano_renderer
from utils import partition, generator_decorator, multi_output_generator_decorator


batched_dot = t.func.vmap(t.dot)


@generator_decorator
def bone_lengths_generator(u, consts):
    """Generate skeleton bone lengths from log-normal model."""
    return t.exp(t.tensor(consts['log_lengths_mean']) +
                  t.dot(u, t.tensor(consts['log_lengths_covar_chol'])))


def joint_angles_cos_sin_vae_decoder(h, layers, n_joint_angle):
    h = layers[0]['nonlinearity'](
        t.dot(h, layers[0]['weights'] + layers[0]['biases']))
    # intermediate layers with skip-connections
    for layer in layers[1:-1]:
        h = layer['nonlinearity'](
            t.dot(h,layer['weights']) + layer['biases']) + h
    h = layers[-1]['nonlinearity'](
        t.dot(h, layers[-1]['weights']) + layers[-1]['biases'])
    return h[:, :n_joint_angle * 2], t.exp(0.5 * h[:, n_joint_angle * 2:])
...
def joint_3d_pos_generator(u, consts):
    ...
    return t.stack(theano_renderer.joint_positions_batch(
...
def joint_3d_pos_var_root_generator(u, consts):
    ...
    joint_pos_3d = t.stack(theano_renderer.joint_positions_batch(
        consts['skeleton'], joint_angles, consts['fixed_joint_angles'],
        lengths=bone_lengths, lengths_map=consts['bone_lengths_map'],
        skip=consts['joints_to_skip']), 2)
    joint_pos_3d[:, :3, :] += root_position[:, :, None]
    #joint_pos_3d = t.inc_subtensor(joint_pos_3d[:, :3, :], root_position[:, :, None])
    return joint_pos_3d
...
def monocular_2d_proj_var_root_generator(u, consts):
    ...
    joint_pos_2d_hom = t.dot(camera_matrix, joint_pos_3d)
...
def binocular_2d_proj_generator(u, consts):
    ...
    cam_mtx_2 = theano_renderer.camera_matrix_batch(
        cam_foc, cam_pos - consts['cam_pos_offset'],
        cam_ang - consts['cam_ang_offset'])
    joint_pos_2d_hom_1 = t.dot(cam_mtx_1, joint_pos_3d)
    joint_pos_2d_1 = (joint_pos_2d_hom_1[:, :2] /
                      joint_pos_2d_hom_1[:, 2][:, None, :])
    joint_pos_2d_hom_2 = t.dot(cam_mtx_2, joint_pos_3d)
...
def joint_3d_pos_generator_hier(state, consts):
    ...
    joint_angles = t.arctan2(joint_ang_cos_sin.T[consts['n_joint_angle']:].T,
                              joint_ang_cos_sin.T[:consts['n_joint_angle']].T)
    bone_lengths = t.exp(log_bone_lengths)
    return tt.squeeze(t.stack(theano_renderer.joint_positions_batch(
        consts['skeleton'], joint_angles, consts['fixed_joint_angles'],
        lengths=bone_lengths, lengths_map=consts['bone_lengths_map'],
        skip=consts['joints_to_skip']), 2))
...
def energy_func_hier_monocular(state, y_data, consts):
    ...
    joint_angles = t.arctan2(joint_ang_cos_sin[consts['n_joint_angle']:],
                              joint_ang_cos_sin[:consts['n_joint_angle']])
    bone_lengths = t.exp(log_bone_lengths)
    joint_pos_3d = t.stack(theano_renderer.joint_positions(
        consts['skeleton'], joint_angles, consts['fixed_joint_angles'],
        lengths=bone_lengths, lengths_map=consts['bone_lengths_map'],
        skip=consts['joints_to_skip']), 1)
    cam_foc = t.exp(consts['cam_foc'])
    cam_pos = t.concatenate([cam_pos_x, cam_pos_y, tt.exp(log_cam_pos_z)])
    cam_ang = consts['cam_ang']
    cam_mtx = theano_renderer.camera_matrix(cam_foc, cam_pos, cam_ang)
    joint_pos_2d_hom = t.dot(cam_mtx,joint_pos_3d)
    joint_pos_2d = joint_pos_2d_hom[:2] / joint_pos_2d_hom[2]
    y_model = t.flatten(joint_pos_2d)
...
def energy_func_hier_binocular(state, y_data, consts):
    ...
    y_model = t.concatenate([t.flatten(joint_pos_2d_1),
                              t.flatten(joint_pos_2d_2)], 0)
    log_lengths_minus_mean = log_bone_lengths - consts['log_lengths_mean']
    return 0.5 * (
        (y_data - y_model).dot(y_data - y_model) /
        consts['output_noise_std']**2 +
        (((joint_ang_cos_sin - ang_cos_sin_mean) / ang_cos_sin_std)**2).sum() +
        t.dot(joint_ang_latent,joint_ang_latent) +
        t.dot(log_lengths_minus_mean,(sla.solve_upper_triangular(
            consts['log_lengths_covar_chol'],
            sla.solve_lower_triangular(
                consts['log_lengths_covar_chol'].T,
                log_lengths_minus_mean)
